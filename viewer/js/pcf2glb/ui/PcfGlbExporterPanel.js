...
export function renderPcfGlbExporterPanel(container) {
  ...
  container.innerHTML = `
    <div style="display: flex; height: calc(100vh - 120px);">
      ...
      <div style="width: 300px; padding: 20px; background: #f0f4f8; border-right: 1px solid #ccc; display: flex; flex-direction: column;">
        ...
        <input type="file" id="pcf-file-input" accept=".pcf" style="margin-bottom: 10px;" ${hasParsedState ? 'disabled' : ''}>
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
         loader.load(currentBlobUrl, (gltf) => {
             ...
             axesHelper.scale.set(maxDim/2, maxDim/2, maxDim/2);

             // Use a multiplier to ensure we back up far enough based on FOV
             const fitHeightDistance = maxDim / (2 * Math.atan(Math.PI * camera.fov / 360));
             const fitWidthDistance = fitHeightDistance / camera.aspect;
             // Use 1.5x multiplier to zoom out slightly more so the edge components fit
             const distance = Math.max(fitHeightDistance, fitWidthDistance) * 1.5;

             const dir = camera.position.clone().sub(center).normalize();
             // Provide a default direction (isometric-ish angle) if already at center
             ...
             // Setup a small point light at the camera to ensure faces aren't entirely black
  ...
  btnDownload.addEventListener('click', () => {
    if (!currentBlobUrl || !currentFile) return;
