...
export const ReliefValveCalc = {
  id: 'mc-rvforce',
  name: 'Gas/Vapour Relief Reaction',
  ...
  run: (envelope) => {
    ...
    envelope.warnings.push('Gas/Vapour discharge only');
    envelope.warnings.push('Not valid for liquid relief');
    envelope.warnings.push('Screening calculation only unless benchmarked');
    envelope.warnings.push('F1 Formula as per API 520 part II is applicable for vapor discharge, not for two phase flow discharge.');
    envelope.warnings.push('No overpressure accumulation on PS is added (non-conservative for F3).');
    
    if (pe_gauge === 0) {
       envelope.warnings.push('PE is ignored / set to Zero because it is less than PA.');
    }
    ...
    envelope.outputs = {
      pe_gauge: UnitSystem.format(pe_gauge, 'pressure', targetMode, 'psi').value,
      force_momentum: UnitSystem.format(force_momentum, 'force', targetMode, 'lbf').value,
      force_pressure: UnitSystem.format(force_pressure, 'force', targetMode, 'lbf').value,
      force_open: UnitSystem.format(force_open, 'force', targetMode, 'lbf').value,
      force_closed: UnitSystem.format(force_closed, 'force', targetMode, 'lbf').value
    };
  }
};
